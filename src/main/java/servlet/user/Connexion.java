	...
	public Connexion() { try { 
		...
		System.out.println("connexion etablie");
			...
			System.err.println("Problème de pilote");
				...
				}    } public void lire(String requête) { try { résultat = instruction.executeQuery(requête);
				}  catch (SQLException ex) {           
					System.err.println("Requête incorrecte "+requête);
					}    } 
				public void miseAJour(String requête) { try { instruction.executeUpdate(requête);
				}  catch (SQLException ex) {
					System.err.println("Requête incorrecte "+requête);
					
					}    } 
				public boolean suivant() { try { 
					return résultat.next();
					} catch (SQLException ex) { 
						return false;
						}    } 
				public void enregistrer(String login,String password) { miseAJour("INSERT INTO users (login,password) VALUES ('"+login+"','"+password+"')");
				}   
		...
		public boolean existeDeja(String login,String password) {       
			lire("SELECT * FROM users where login = '"+login+"' and password = '"+password+"'"); return suivant();
			} public void arrêt() { try { connexion.close();
			}  catch (SQLException ex) {          
